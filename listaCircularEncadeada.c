/************************************************************************************
/* Programa para criação de Estrutura de Dados - lista circular duplamente encadeada
...
void inserirInicio(struct caixa** listaLocal, int add){
    ...
    if (*listaLocal == 0){                                              // se a lista estiver vazia, lista aponta para elemento inserido.
        ...
        novo -> prox = novo;
        *listaLocal = novo;
    } else if (((*listaLocal)->ant == (*listaLocal)->prox) && (*listaLocal)->prox == (*listaLocal)){
    	novo -> prox = *listaLocal;                                     
        novo -> ant = *listaLocal;
        (*listaLocal)->ant = novo;
        (*listaLocal)->prox = novo;
        //*listaLocal = novo;
    } else {
        novo -> prox = *listaLocal;                                     
        novo -> ant = (*listaLocal)->ant;
        (*listaLocal)->ant->prox = novo;                                                
        (*listaLocal)->ant = novo;								
        //*listaLocal = novo;                                             
    }
}
...
void listar(struct caixa *listaLocal){
    ...
    if (listaLocal == 0){
        ...
        struct caixa *aux = listaLocal;
        
        do{
        	printf("%d\n",aux -> dado);
        	aux = aux -> prox;
        } while (aux != listaLocal);
